...
void ReplaceDialog::on_Change_clicked()
{
    QString target=ui->searchEdit->text();
...
void ReplaceDialog::on_AllChange_clicked()
{
    QString target=ui->searchEdit->text();
    QString to=ui->targetText->text();
    if((pTextEdit!=nullptr)&&(target!="")&&(to!="")){
        QString text=pTextEdit->toPlainText();
        text.replace(target,to,ui->BigSmall->isChecked()?Qt::CaseSensitive:Qt::CaseInsensitive);
        pTextEdit->clear();
